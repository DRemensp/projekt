...
<div class="max-w-full">
    <form action="{{ route('scoresystem.store') }}" method="POST" class="space-y-6">
        ...
        {{-- Platz 1-3 Section --}}
        <div class="bg-white night-panel dark:bg-gray-800 p-4 rounded-xl border border-gray-300 dark:border-gray-600 transition-colors duration-300">
        ...
        {{-- Schulwertung - Teams pro Schule --}}
        <div class="bg-white night-panel dark:bg-gray-800 p-4 rounded-xl border border-gray-300 dark:border-gray-600 transition-colors duration-300"
             x-data="{ manual: {{ ($scoresystem && $scoresystem->school_teams_override !== null) ? 'true' : 'false' }} }">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-4 flex items-center gap-2">
            ...
            <div class="flex items-center gap-4 mb-4">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="_school_teams_mode" value="auto" x-model="manual" :value="false" @change="manual = false" class="text-indigo-600" {{ ($scoresystem && $scoresystem->school_teams_override !== null) ? '' : 'checked' }}>
                    <span class="text-sm text-gray-700 dark:text-gray-300">
                        Auto
                        @if($autoSchoolTeams !== null)
                            <span class="ml-1 text-xs text-gray-500 dark:text-gray-400">(aktuell: {{ $autoSchoolTeams }} Teams)</span>
                        @else
                            <span class="ml-1 text-xs text-gray-500 dark:text-gray-400">(noch keine Teams)</span>
                        @endif
                    </span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="_school_teams_mode" value="manual" x-model="manual" :value="true" @change="manual = true" class="text-indigo-600" {{ ($scoresystem && $scoresystem->school_teams_override !== null) ? 'checked' : '' }}>
                    <span class="text-sm text-gray-700 dark:text-gray-300">Manuell</span>
                </label>
            </div>

            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                Im Auto-Modus wird automatisch die kleinste Teamanzahl aller Schulen verwendet, damit alle gleich viele Teams in die Wertung einbringen.
            </p>

            <div x-show="manual" x-transition>
                <label for="school_teams_override" class="block text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">
                    Anzahl Teams pro Schule (fix)
                </label>
                <input
                    type="number"
                    name="school_teams_override"
                    id="school_teams_override"
                    placeholder="z.B. 3"
                    min="1"
                    :disabled="!manual"
                    value="{{ old('school_teams_override', $scoresystem->school_teams_override ?? '') }}"
                    class="w-full px-4 py-3 bg-white dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-xl shadow-sm placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-300 text-sm"
                >
                @error('school_teams_override')
                <div class="flex items-center gap-2 mt-2 text-red-600 dark:text-red-400">
                    <span class="text-xs font-medium">{{ $message }}</span>
                </div>
                @enderror
