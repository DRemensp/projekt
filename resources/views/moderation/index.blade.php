    ...
    <div class=" bg-gradient-to-br from-blue-100 to-green-100 mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ url('/') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    <div class="text-3xl font-bold mb-1 text-gray-800">{{ $stats['total'] }}</div>
                    <div class="text-sm text-gray-600">Gesamt</div>
                </div>
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    <div class="text-3xl font-bold mb-1 text-gray-800">{{ $stats['approved'] }}</div>
                    <div class="text-sm text-gray-600">Freigegeben</div>
                </div>
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    <div class="text-3xl font-bold mb-1 text-gray-800">{{ $stats['pending'] }}</div>
                    <div class="text-sm text-gray-600">Ausstehend</div>
                </div>
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    <div class="text-3xl font-bold mb-1 text-gray-800">{{ $stats['blocked'] }}</div>
                    <div class="text-sm text-gray-600">Blockiert</div>
                </div>
            </div>
        </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nachricht</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">IP-Adresse</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">AI-Scores</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Grund</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Datum</th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-medium text-gray-900">{{ $comment->author_name ?? 'Anonym' }}</span>
                                    </div>
                                </td>

                                <!-- Nachricht -->
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 max-w-md break-words">{{ $comment->message }}</div>
                                </td>

                                <!-- IP-Adresse -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <code class="text-xs bg-gray-100 px-2 py-1 rounded font-mono text-gray-700">{{ $comment->ip_address ?? 'N/A' }}</code>
                                </td>
                                        ...
                                        <div class="space-y-1">
                                            @foreach($comment->moderation_scores as $attribute => $score)
                                                <div class="flex items-center gap-2 text-xs">
                                                    <span class="font-medium text-gray-600 w-32 truncate" title="{{ $attribute }}">{{ $attribute }}</span>
                                                    ...
                                                    <span class="font-mono text-gray-700 w-12 text-right">{{ number_format($score * 100, 1) }}%</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    @if($comment->moderation_reason)
                                        <div class="text-xs text-gray-600 max-w-xs">{{ $comment->moderation_reason }}</div>
                                    @else
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if($comment->moderation_status !== 'approved')
                                            <form action="{{ route('moderation.approve', $comment) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:text-green-700 p-1 hover:bg-green-50 rounded transition-colors" title="Freigeben">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                    </svg>
                                                </button>
                                        ...
                                        @if($comment->moderation_status !== 'blocked')
                                            <form action="{{ route('moderation.block', $comment) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="text-yellow-600 hover:text-yellow-700 p-1 hover:bg-yellow-50 rounded transition-colors" title="Blockieren">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                                                    </svg>
                                                </button>
                                        ...
                                        <form action="{{ route('moderation.destroy', $comment) }}" method="POST" onsubmit="return confirm('Kommentar wirklich löschen?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-700 p-1 hover:bg-red-50 rounded transition-colors" title="Löschen">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-gray-500 font-medium">Noch keine Kommentare vorhanden</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
