<div class="max-w-3xl mx-auto my-8">
    ...
    <script>
        // Globale Namespace für Comments verwenden
        window.CommentSystem = window.CommentSystem || {};

        // Prüfung der Cookie-Einstellungen für Moderation
        if (!window.CommentSystem.checkModerationCookies) {
            window.CommentSystem.checkModerationCookies = function() {
                try {
        ...
        if (!window.CommentSystem.updateCommentDisplay) {
            window.CommentSystem.updateCommentDisplay = function() {
                const moderationAllowed = window.CommentSystem.checkModerationCookies();
                const commentForms = document.querySelectorAll('#comment-form-container');
                const blockedMessages = document.querySelectorAll('#moderation-blocked-message');

                commentForms.forEach(form => {
                    if (form) form.style.display = moderationAllowed ? 'block' : 'none';
                });

                blockedMessages.forEach(msg => {
                    if (msg) msg.style.display = moderationAllowed ? 'none' : 'block';
                });
            };
        }

        // Initialisierung nur einmal durchführen
        if (!window.CommentSystem.isInitialized) {
            window.CommentSystem.isInitialized = true;

            function initializeCommentDisplay() {
                window.CommentSystem.updateCommentDisplay();

                // Event Listeners nur einmal registrieren
                window.addEventListener('cookiePreferencesUpdated', window.CommentSystem.updateCommentDisplay);
                document.addEventListener('livewire:navigated', window.CommentSystem.updateCommentDisplay);
    ...
    @if($commentsEnabled)
    <div id="comment-form-container" wire:ignore style="display: none;" class="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl border border-blue-100 dark:border-gray-700 shadow-sm transition-colors duration-300">
        ...
        @if (session()->has('comment_success'))
            <div class="mb-4 p-3 bg-green-100 dark:bg-green-900 border border-green-300 dark:border-green-700 text-green-700 dark:text-green-200 rounded-lg text-sm transition-colors duration-300">
                {{ session('comment_success') }}
            </div>
        @endif

        @if (session()->has('comment_pending'))
            <div class="mb-4 p-3 bg-yellow-100 dark:bg-yellow-900 border border-yellow-300 dark:border-yellow-700 text-yellow-700 dark:text-yellow-200 rounded-lg text-sm transition-colors duration-300">
        ...
        @if (session()->has('comment_blocked'))
            <div class="mb-4 p-3 bg-red-100 dark:bg-red-900 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-200 rounded-lg text-sm transition-colors duration-300">
                <div class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <form wire:submit.prevent="store" class="space-y-4">
            <!-- Name Eingabe -->
            <div>
                <label for="authorName" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                    Name (optional)
                </label>
                <input
                    ...
                    type="text"
                    id="authorName"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-300"
            ...
            <div>
                <label for="message" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                    Nachricht <span class="text-red-500 dark:text-red-400">*</span>
                </label>
                <input
    ...
    <div id="moderation-blocked-message" wire:ignore.self style="display: none;" class="mb-8 bg-gradient-to-r from-orange-50 to-red-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl border border-orange-200 dark:border-gray-700 shadow-sm transition-colors duration-300">
        <div class="text-center">
        ...
        <div class="space-y-4">
                ...
                @auth
                    @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('teacher'))
                        <a href="{{ route('moderation.index') }}"
                           class="inline-flex items-center gap-2 px-4 py-2 bg-gray-700 dark:bg-gray-600 text-white text-sm font-medium rounded-lg hover:bg-gray-800 dark:hover:bg-gray-700 transition-colors duration-300 shadow-sm hover:shadow-md">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="space-y-3">
                @foreach($comments as $comment)
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-300">
                            ...
                            @auth
                                @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('teacher'))
                                    <button
                                        ...
                                        class="ml-2 text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-colors duration-300 p-1 rounded hover:bg-red-100 dark:hover:bg-red-900 flex-shrink-0"
                    ...
                    <div class="text-center py-4 space-y-3">
                        ...
                        @if($commentsToShow > 5)
                            <div>
                                <button
                                    wire:click="showLess"
                                    class="inline-flex items-center gap-2 px-4 py-2 text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 font-medium text-sm bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition-colors duration-300"
    ...
    @else
        <div class="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors duration-300">
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm">Sei der Erste, der einen Kommentar schreibt!</p>
        </div>
    @endif
</div>
